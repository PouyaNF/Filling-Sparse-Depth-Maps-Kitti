...
if __name__ == '__main__':
    ...
    loader = Kittiloader(kitti_root_path,  'test')
    data_size = loader.data_length()  # get data split size
    print(data_size)
    data_item = loader.load_item(1)  # which is very suitable for pytorch dataloader
    print(len(data_item))
    image = data_item['img']
    print(type(image))
    plt.imshow(image)
    ...
    plt.imshow(depth)
    plt.show()
    filled_depth = data_item['depth_interp']
    print(type(filled_depth))
